...
const validateFields = (fields) => {
  ...
  for (const [sectionName, sectionFields] of Object.entries(fields)) {
    if (typeof sectionFields !== "object" || Object.keys(sectionFields).length === 0) {
      throw new Error(`Section ${sectionName} must be a non-empty object`);
    }

    for (const [fieldName, fieldProps] of Object.entries(sectionFields)) {
      if (typeof fieldProps !== "object" || !fieldProps.type || !fieldProps.label) {
        throw new Error(`Field ${fieldName} in section ${sectionName} is invalid`);
      }
    }
  }
};
...
const getAllAssessmentTemplates = async (req, res) => {
  try {
    ...
    res.status(StatusCodes.OK).json({ assessmentTemplates: formattedTemplates });
  } catch (error) {
    console.error(error);
    errorHandler(res, StatusCodes.INTERNAL_SERVER_ERROR, "Server Error");
  }
